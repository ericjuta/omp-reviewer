...
async function configFile(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "omp-reviewer-config-"));
  cleanup.push(root);
  return path.join(root, "nested", "config.json");
}

describe("user config", () => {
  it("uses XDG_CONFIG_HOME and defaults to OMP authentication", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "omp-reviewer-xdg-"));
    cleanup.push(root);
    const file = defaultConfigPath({ XDG_CONFIG_HOME: root });
    expect(file).toBe(path.join(root, "omp-reviewer", "config.json"));
    await expect(loadConfig(file)).resolves.toEqual({ version: 1, auth: "omp" });
  });
  ...
  it("rejects malformed, isolated, and unknown config fields", async () => {
    ...
    expect(() => validateConfig({ version: 1, auth: "omp", extra: true })).toThrow("unknown field");
    expect(validateConfig({ version: 1 })).toEqual({ version: 1, auth: "omp" });
    expect(() => validateConfig({ version: 1, auth: "isolated" })).toThrow("auth must be omp");
    expect(() => validateConfig({ version: 1, auth: "omp", model: 1 })).toThrow(
      "model must be a string",
    );
    expect(() => validateConfig({ version: 1, auth: "omp", thinking: "extreme" })).toThrow(
      "thinking must",
    );
    const file = await configFile();
    await writeConfig({ version: 1, auth: "omp" }, file);
